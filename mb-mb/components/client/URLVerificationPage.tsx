import { Phone, Globe, Link2, AlertTriangle } from 'lucide-react';
...
export default function URLVerificationPage() {
  ...
  const { submitURL, setCurrentPage } = useSession();
    ...
    <div className="min-h-screen bg-[#1a1d23] text-white">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-16">
        <div className="bg-[#242830] border border-gray-700 rounded-lg overflow-hidden">
          <div className="bg-[#2a3040] px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-700">
            <h1 className="text-white text-lg sm:text-xl md:text-2xl">{t('urlVerification')}</h1>
            <p className="text-gray-400 mt-1 text-sm sm:text-base">{t('enterVerificationURL')}</p>
          </div>

          <div className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
            {/* Instructions */}
            <div className="bg-[#1a1d23] rounded-lg p-4 sm:p-6 border-l-4 border-[#d4af37]">
              <h3 className="text-white mb-2 sm:mb-3 flex items-center gap-2 text-sm sm:text-base">
                <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-[#d4af37]" />
                {t('urlInstructions')}
              </h3>
              <ol className="space-y-1.5 sm:space-y-2 text-gray-300 text-xs sm:text-sm">
                <li className="flex gap-2">
                  <span className="text-[#d4af37] flex-shrink-0">1.</span>
                  <span>{t('urlInstruction1')}</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-[#d4af37] flex-shrink-0">2.</span>
                  <span>{t('urlInstruction2')}</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-[#d4af37] flex-shrink-0">3.</span>
                  <span>{t('urlInstruction3')}</span>
                </li>
              </ol>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-11 h-11 sm:h-12 bg-[#1a1d23] border-gray-600 text-white placeholder:text-gray-500 text-sm sm:text-base"
                    required
                  />
                </div>
              </div>
