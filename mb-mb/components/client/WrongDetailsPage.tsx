...
export default function WrongDetailsPage() {
  const { toggleLanguage, t } = useLanguage();
  const { setCurrentPage } = useSession();
  ...
  return (
    <div className="min-h-screen bg-[#1a1d23] text-white">
      ...
      <main className="max-w-md mx-auto px-6 py-16">
        <div className="bg-[#242830] border border-red-700 rounded-lg overflow-hidden">
          <div className="bg-red-900/30 px-8 py-6 flex items-center gap-3 border-b border-red-700">
            <AlertTriangle className="w-6 h-6 text-red-500" />
            <h1 className="text-white">{t('verificationFailed')}</h1>
          </div>
          
          <div className="p-8 space-y-6">
            <div className="text-center space-y-4">
              <div className="bg-red-900/20 border-2 border-red-700 rounded-full w-20 h-20 flex items-center justify-center mx-auto">
                <AlertTriangle className="w-10 h-10 text-red-500" />
              </div>
              
              <h2 className="text-white">{t('incorrectInformation')}</h2>
            ...
            <div className="bg-[#1a1d23] rounded-lg p-6 border-l-4 border-red-500">
              <p className="text-gray-300 leading-relaxed">
                <strong className="text-white block mb-2">{t('whatHappensNext')}</strong>
                {t('securityTeamReviewing')}
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="text-center">
                <p className="text-gray-400 text-sm">
                  {t('forAssistance')} <a href="tel:224443636" className="text-[#d4af37] hover:underline">224 443 636</a>
                </p>
              </div>
