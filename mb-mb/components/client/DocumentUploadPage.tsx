import { Phone, Globe, Upload, FileText, Camera, CheckCircle2 } from 'lucide-react';
...
export default function DocumentUploadPage() {
  ...
  const { submitDocumentData } = useSession();
  ...
  const handleFileChange = (type: 'idFront' | 'idBack' | 'selfie', file: File) => {
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-lg p-6 sm:p-8 text-center cursor-pointer transition-all ${
          uploaded
            ...
            : 'border-gray-600 bg-[#1a1d23] hover:border-[#d4af37] hover:bg-[#1a1d23]/80'
        }`}
      >
        {uploaded ? (
          <CheckCircle2 className="w-10 h-10 sm:w-12 sm:h-12 mx-auto text-green-500 mb-2" />
        ) : (
          <Icon className="w-10 h-10 sm:w-12 sm:h-12 mx-auto text-gray-400 mb-2" />
        )}
        <p className={`text-sm sm:text-base ${uploaded ? 'text-green-500' : 'text-gray-400'}`}>
          {uploaded ? '✓ ' + t('uploadDocument') : t('clickToUpload')}
        </p>
        {!uploaded && (
          <p className="text-xs text-gray-500 mt-1">{t('orDragAndDrop')}</p>
        )}
      </div>
    </div>
  );
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-16">
        <div className="bg-[#242830] border border-gray-700 rounded-lg overflow-hidden">
          <div className="bg-[#2a3040] px-4 sm:px-8 py-4 sm:py-5 flex items-center gap-3 border-b border-gray-700">
            ...
            <div>
              <h1 className="text-white text-lg sm:text-xl md:text-2xl">{t('documentUpload')}</h1>
              <p className="text-gray-400 text-xs sm:text-sm mt-0.5">{t('uploadDocuments')}</p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
            {/* Upload Instructions */}
            <div className="bg-[#1a1d23] rounded-lg p-4 sm:p-6 border-l-4 border-[#d4af37]">
              <h3 className="text-white mb-2 sm:mb-3 text-sm sm:text-base">{t('uploadInstructions')}</h3>
              <ul className="space-y-1 sm:space-y-1.5 text-gray-300 text-xs sm:text-sm">
                ...
                <li className="flex items-start gap-2">
                  <span className="text-[#d4af37] mt-0.5">•</span>
                  <span>{t('uploadInstruction2')}</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-[#d4af37] mt-0.5">•</span>
                  <span>{t('uploadInstruction3')}</span>
                </li>
              </ul>
            </div>
            ...
            <Button
              type="submit"
              disabled={!uploads.idFront || !uploads.idBack || !uploads.selfie}
              className="w-full bg-[#d4af37] hover:bg-[#c09d2e] text-black disabled:opacity-50 disabled:cursor-not-allowed h-11 sm:h-12 text-sm sm:text-base"
            >
              {t('continue')}
            </Button>
          </form>
